...
export default async function ReportsPage() {
  ...
  events.forEach(event => {
    ...
    const month = new Date(event.date).toLocaleString('default', { month: 'short', year: '2-digit' });
    
    if (!monthlyData[month]) {
      monthlyData[month] = { revenue: 0, profit: 0 };
    }

    const eventCost = event.expenses.reduce((sum, exp) => sum + exp.actualAmount, 0);
  ...
  const expenseChartData = Object.entries(expenseCategoryData).map(([name, value]) => ({
    name,
    value,
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-md border bg-white p-6">
        ...
        <ul className="list-disc list-inside space-y-2 text-gray-600">
          <li>Top revenue month: <strong>{revenueChartData.sort((a,b) => b.value - a.value)[0]?.name || 'N/A'}</strong></li>
          <li>Most expensive category: <strong>{expenseChartData[0]?.name || 'N/A'}</strong></li>
          <li>Total Profit Year-to-Date: <strong>{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(profitChartData.reduce((a,b) => a + b.value, 0))}</strong></li>
        </ul>
      </div>
    </div>
  );
